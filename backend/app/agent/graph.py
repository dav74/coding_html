...
generator_llm = llm.with_structured_output(SiteOutput)
reviewer_llm = llm.with_structured_output(ReviewOutput)


def _try_parse_site_json(text: str) -> SiteOutput | None:
    try:
...
async def generate(state: GraphState) -> GraphState:
    ...
    try:
        ...
        fallback = _try_parse_site_json(str(e))
        if fallback:
            return {**state, "html": fallback.html, "css": fallback.css}
        return {
            **state,
            "status": "failed",
